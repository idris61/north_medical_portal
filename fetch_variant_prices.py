...
def get_all_products_with_variants():
    """Web sitesinden TÜM ürünleri çek (varyant fiyatları dahil)"""
        ...
        try:
            ...
            for product_elem in wc_products:
                ...
                if prices:
                    ...
                    for price in prices:
                        if not any(p['name'] == product_name and abs(p['price'] - price) < 0.01 for p in all_products):
                            all_products.append({
                                'name': product_name,
                                'price': price
                            })
...
def update_all_prices_with_variants():
    ...
    products = get_all_products_with_variants()
    print(f"\n📦 Toplam {len(products)} ürün/varyant fiyatı bulundu")
        ...
        if matched_item and score >= 0.5:
            ...
            if existing_price:
                ...
                if abs(existing_price - price) > 0.01:
                    ...
                    print(f"✅ {matched_item.item_code}: {existing_price}€ → {price}€ (skor: {score:.2f})")
                else:
                ...
                item_price.insert()
                print(f"➕ {matched_item.item_code}: {price}€ (yeni, skor: {score:.2f})")
    ...
    variants_without_price = frappe.db.sql("""
        ...
        AND ip.name IS NULL
        LIMIT 10
    """, (price_list_name, currency), as_dict=True)
...
if __name__ == "__main__":
    update_all_prices_with_variants()
